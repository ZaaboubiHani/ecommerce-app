    ...
    if (response.status === 200) {
      const combinedData = uniqueWilayaNamesArray.map((wilaya) => {
        const priceData = response.data.find(
          (price) => price.name === wilaya.wilaya_name_ascii
        )
        return {
          ...wilaya,
      ...
      {loading ? (
        ...
        duration-300'
          >
            <div
              className={`${
                selectedWilaya?.arWilaya ? 'text-black' : 'text-gray-500'
              }`}
            >
              {language === 'ar'
                ? selectedWilaya?.arWilaya ?? 'الولاية'
                : language === 'fr'
                ? selectedWilaya?.frWilaya ?? 'ًWilaya'
                : selectedWilaya?.enWilaya ?? 'ًWilaya'}
            </div>
