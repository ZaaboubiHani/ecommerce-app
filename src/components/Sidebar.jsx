  ...
  shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-50 px-4 lg:px-[35px]`}
    >
      ...
      <div className='flex flex-col gap-y-3 py-4'>
        ...
        <Link
          to='/'
          className='bg-gray-500 w-full flex p-4 justify-center items-center text-white font-medium text-nowrap rounded-2xl'
        >
          {language === 'ar'
            ? 'اشتري اكثر'
            : language === 'fr'
      ...
      ${cart.length > 0 ? 'opacity-100' : 'opacity-50'} ${
            cart.length > 0 ? 'cursor-pointer' : 'cursor-not-allowed'
          }`}
        >
          {language === 'ar' ? 'طلب' : language === 'fr' ? 'Paiement' : 'Checkout'}
        </Link>
      </div>
    </div>
  )
