    ...
    <div className="relative">
      ...
      <div className="flex w-full justify-center mt-2">
        {products.map((_, i) => (
          <div
            key={i}
            className={`h-2 w-2 rounded-full cursor-pointer mr-4 ${i === currentIndex ? 'bg-black' : 'bg-gray-400'}`}
      ...
      <div className="absolute h-full top-0 flex items-center right-0">
        <MdOutlineArrowForwardIos
          className={`text-4xl text-white cursor-pointer ${currentIndex === products.length - 1 ? 'opacity-50' : 'opacity-100'}`}
          onClick={handleNext}
        />
      </div>
      <div className="absolute h-full top-0 flex items-center left-0">
        <MdArrowBackIosNew
          className={`text-4xl text-white cursor-pointer ${currentIndex === 0 ? 'opacity-50' : 'opacity-100'}`}
          onClick={handlePrev}
        />
      </div>
    </div>
